...
local composition =
  ...
  common.VshnMetaDBaaSExoscale('PostgreSQL') + {
    metadata+: {
      annotations+: {
        'metadata.appcat.vshn.io/plans': importstr 'exoscale-plans/pg.json',
      },
    },
  } +
  {
...
if params.services.exoscale.enabled && pgParams.enabled && vars.isSingleOrControlPlaneCluster then {
